...
const appID: string =
  COMETCHAT_CONSTANTS.APP_ID || (localStorage.getItem("appId") ?? ""); // Use the latest appId if available
const region: string =
  COMETCHAT_CONSTANTS.REGION || (localStorage.getItem("region") ?? ""); // Default to 'us' if region is not found
const authKey: string =
  COMETCHAT_CONSTANTS.AUTH_KEY || (localStorage.getItem("authKey") ?? ""); // Default authKey if not found
...
// Web vitals disabled in production build to avoid missing dependency/types
