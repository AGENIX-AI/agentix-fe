      ...
      <TooltipTrigger asChild>
        <div
          ...
          onMouseDown={(e) => {
            ...
            // Only set selection for blocks with valid IDs
            ...
            editor
              .getApi(BlockSelectionPlugin)
              .blockSelection.set(validBlockIds);
          }}
          onMouseEnter={() => {
            if (isDragging) return;
...
const createDragPreviewElements = (
  ...
  // Early return if no blocks or invalid blocks array
  ...
  const resolveElement = (node: TElement, index: number) => {
    ...
    // Skip this node if DOM node is not available
    if (!domNode) {
      return;
    }
    ...
    if (lastDomNode && index > 0) {
      const lastDomNodeElement = editor.api.toDOMNode(lastDomNode);

      // Only proceed if we can get the DOM node
      if (lastDomNodeElement && domNode.parentElement) {
        const lastDomNodeRect =
          lastDomNodeElement.parentElement!.getBoundingClientRect();
  ...
  // Filter out any undefined results and only keep valid elements
  const validElements = elements.filter(Boolean);
  const validIds = ids.filter(Boolean);

  editor.setOption(DndPlugin, "draggingId", validIds);
...
const calculatePreviewTop = (
  ...
  // Early return if any required DOM nodes are not available
  ...
  // Early return if first DOM node is not available
  if (!firstDomNode) {
    return 0;
  }
...
const calcDragButtonTop = (editor: PlateEditor, element: TElement): number => {
  ...
  // Early return if DOM node is not available
  if (!child) {
    return 0;
  }
