...
export const CometChatCallDetails = (props: {
  ...
  useEffect(() => {
    ...
    setSubtitleText(
      getLocalizedString(
        `call_logs_user_status_${user?.getStatus().toLowerCase()}`
      )
    );
      ...
      new CometChat.UserListener({
        ...
        onUserOffline: (offlineUser: CometChat.User) => {
          if (user?.getUid() === offlineUser?.getUid()) {
            setSubtitleText(
              getLocalizedString(`call_logs_user_status_offline`)
            );
          }
        },
      })
    ...
    return (
      <div className="cometchat-call-logs-details__header__shimmer__shimmer">
        {[...Array(1)].map((_, index) => (
          <div
            key={index}
            className="cometchat-call-logs-details__header__shimmer-item"
          >
