...
function CometChatHome(props: { theme?: "light" | "dark" }) {
  ...
  const [selectedItem, setSelectedItem] = useState<
    CometChat.Conversation | CometChat.User | CometChat.Group | CometChat.Call
  >();
  ...
  // If a theme prop is provided, treat it as the source of truth.
  // Only fall back to system preference when no explicit theme is passed.
  useEffect(() => {
    if (props.theme) {
  ...
  const updateUserAfterBlockUnblock = (user: CometChat.User) => {
    ...
    if (
      (
        appState.selectedItem?.getConversationWith() as CometChat.User
      )?.getUid?.() === user.getUid()
    ) {
    ...
    return (
      <>
        {(selectedItem as any)?.mode === "call" ? (
          ...
          />
        ) : messageUser && messageUser.getRole() == "@agentic" ? (
          <CometChatAIAssistantChat
            user={messageUser}
            ...
            <CometChatMessages
              ...
              goToMessageId={
                appState.threadSearchMessage
                  ? undefined
                  : appState.goToMessageId
              }
              searchKeyword={
                appState.goToMessageId ? appState.searchKeyword : undefined
              }
            />
          )
        )}
    ...
    return (
      <div className="cometchat-new-chat-view">
        ...
        <div className="cometchat-new-chat-view__header">
          ...
          />
          <div className="cometchat-new-chat-view__header-title">
            {getLocalizedString("new_chat_title")}
          </div>
        </div>
  ...
  const SideComponent = () => {
    ...
    const onSearchMessageClicked = async (
      message: CometChat.BaseMessage,
      ...
      if (message.getParentMessageId()) {
        let msg = await CometChat.getMessageDetails(
    ...
    return (
      <>
        {appState.sideComponent.visible || appState.showMessagesSearch ? (
          <div
            className={`side-component-wrapper ${
              appState.threadSearchMessage
          ...
          >
            <div className="side-component-wrapper__content">
              {appState.sideComponent.type == "user" && user && (
                <div
                  className={`side-component-wrapper__content-view ${
                    appState.sideComponentTop === "user"
              ...
              {appState.sideComponent.type == "group" && group && (
                <div
                  className={`side-component-wrapper__content-view ${
                    appState.sideComponentTop === "group"
                      ? "side-component-wrapper__content-top"
                      : "side-component-wrapper__content-bottom"
                  }`}
                >
              ...
              appState.threadedMessage ? (
                <div
                  className={`side-component-wrapper__content-view ${
                    appState.sideComponentTop === "threadedMessage"
                      ? "side-component-wrapper__content-top"
                      : "side-component-wrapper__content-bottom"
                  }`}
                >
              ...
              {appState.showMessagesSearch && (
                <div
                  className={`side-component-wrapper__content-view ${
                    appState.sideComponentTop === "search"
                      ? "side-component-wrapper__content-top"
                      : "side-component-wrapper__content-bottom"
                  }`}
                >
  ...
  const SideComponentThread = (props: ThreadProps) => {
    ...
    return (
      <CometChatThreadedMessages
        ...
        showComposer={showComposer}
        goToMessageId={
          appState.threadSearchMessage && appState.goToMessageId
            ? appState.goToMessageId
            : undefined
        }
        onSubtitleClicked={() => {
          ...
          setAppState({
            type: "updateThreadSearchMessage",
            payload: undefined,
          });
          setAppState({
            type: "updateGoToMessageId",
  ...
  const onSelectorItemClicked = (
    ...
    if (
      type === "updateSelectedItemGroup" &&
      !(e as CometChat.Group).getHasJoined()
    ) {
      if (
        (e as CometChat.Group).getType() ===
        CometChatUIKitConstants.GroupTypes.public
      ) {
        CometChat.joinGroup(
          (e as CometChat.Group).getGuid(),
          (e as CometChat.Group).getType() as CometChat.GroupType
        )
          .then((response: any) => {
            setAppState({
              type: "updateSideComponent",
    ...
    const ccClickEvent = CometChatUIEvents.ccMouseEvent.subscribe(
      (mouseevent: IMouseEvent) => {
        if (
          ...
          (
            mouseevent.body as {
              CometChatUserGroupMembersObject: CometChat.User;
            }
  ...
  const resetSideComponent = (conversationId: string) => {
    if (
      selectedItem &&
      selectedItem instanceof CometChat.Conversation &&
      (selectedItem as CometChat.Conversation).getConversationId() !==
        conversationId
    ) {
      setAppState({
        type: "updateSideComponent",
