...
export function processMessageContent(content: string): {
	...
	while (i < lines.length) {
		...
		if (headingMatch) {
			...
			processedLines.push(
				`<h${level} class="${headingClasses[level as keyof typeof headingClasses]}">${headingText}</h${level}>`,
			);
			i++;
			continue;
		}
		...
		if (numberedListMatch || bulletListMatch) {
			...
			while (j < lines.length) {
				...
				const currentNumberedMatch =
					currentLine.match(/^(\d+)\.\s+(.*)/);
				const currentBulletMatch =
				...
				while (k < lines.length) {
					...
					const nestedNumberedMatch =
						nestedLine.match(/^\s+(\d+)\.\s+(.*)/);
					const nestedBulletMatch =
						nestedLine.match(/^\s+[-*+]\s+(.*)/);
				...
				if (nestedItems.length > 0) {
					...
					if (nestedNumberedList || nestedBulletList) {
						...
						for (const nestedItem of nestedItems) {
							const nestedNumbered =
								nestedItem.match(/^\s+(\d+)\.\s+(.*)/);
							const nestedBullet =
								nestedItem.match(/^\s+[-*+]\s+(.*)/);
