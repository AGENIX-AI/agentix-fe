...
export type WebDerivedKnowledgeDocumentType = "crawl_collection";

interface WebDerivedKnowledgeCollectionsComponentProps {
  refreshTrigger?: number;
  onAddWebDerivedKnowledge?: () => void;
  onSelectCollection?: (collection: Document) => void;
}

export function WebDerivedKnowledgeCollectionsComponent({
  refreshTrigger,
  onAddWebDerivedKnowledge,
  onSelectCollection,
}: WebDerivedKnowledgeCollectionsComponentProps) {
  ...
  useEffect(() => {
    const fetchDocuments = async () => {
      ...
      try {
        ...
        console.error(
          "Error fetching web derived knowledge collections:",
          error
        );
        toast.error("Failed to fetch web derived knowledge collections");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleRowClick = (documentId: string) => {
    try {
      ...
      console.error("Error selecting web derived knowledge collection:", error);
      toast.error("Failed to select web derived knowledge collection");
    }
  };
    ...
    <div className="">
      <div className="mb-3 flex items-center justify-between">
        <Small className="font-semibold">
          {t("documents.webKnowledge.collections")}
        </Small>
        <Button
          onClick={() => {
            onAddWebDerivedKnowledge?.();
        ...
        >
          ...
          {t("documents.webKnowledge.addCollection")}
        </Button>
      </div>
      ...
      <div className="mb-4">
        <div className="relative flex-1">
          ...
          <Input
            type="text"
            placeholder="Search web derived knowledge collections..."
            value={searchQuery}
          ...
          {documents.length === 0 ? (
            <div className="text-center py-8 border rounded-lg">
              <h3 className="mt-2 text-xs font-medium">
                No web derived knowledge collections found
              </h3>
              <p className="text-xs text-muted-foreground mt-1">
                No web derived knowledge collections available
              </p>
            </div>
          ) : (
